...
class Directory(Base):
    ...
    parent_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("directories.id", ondelete="RESTRICT"), nullable=True
    )
    ...
    # Optimization
    __table_args__ = (
        Index("ix_directories_workspace_parent", "workspace_id", "parent_id"),

        # Prevent duplicate sibling directory names within the same parent/workspace context
        UniqueConstraint("workspace_id", "parent_id", "name", name="uq_directories_workspace_parent_name"),
    )
